...
void listen( uvw::loop &loop )
{
    ...
    tcp->on<uvw::listen_event>(
        []( const uvw::listen_event &, uvw::tcp_handle &srv )
        {
            ...
            client->on<uvw::close_event>(
                [ptr = srv.shared_from_this()]( const uvw::close_event &, uvw::tcp_handle & )
                {
            ...
            client->on<uvw::data_event>(
                []( const uvw::data_event &event, uvw::tcp_handle & )
                {
    ...
    tcp->on<uvw::close_event>( []( const uvw::close_event &, uvw::tcp_handle & ) { std::cout << "close" << std::endl; } );
...
void conn( uvw::loop &loop )
{
    ...
    tcp->on<uvw::write_event>(
        []( const uvw::write_event &, uvw::tcp_handle &handle )
        {
